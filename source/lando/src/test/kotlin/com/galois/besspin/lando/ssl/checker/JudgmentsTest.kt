...
class JudgmentsTest : TestCase() {
    ...
    fun testCheckIntroduceElements() {
        ...
        val ret0 = judgments.checkIntroduceElementsFirstPass(ctx, ctx, phi, rel, body0)
        assert(ret0 is CheckStatus.Ok)

        /* check with invalid element */
        val body1 = listOf(generateSubsystem("My System", "Abbrev", body = listOf(TestElement(0, RawPos(1, 1)))))
        val ret1 = judgments.checkIntroduceElementsFirstPass(ctx, ctx, phi, rel, body1)
        assert(ret1 is CheckStatus.Error)
    }
    ...
    fun testCheckIntroduceSubsystem() {
        ...
        /* check simple case is ok */
        val subsys0 = generateSubsystem("My System", "Abbrev")
        val ret0 = judgments.checkIntroduceSubsystem(ctx, ctx, phi, rel, subsys0, true)
        assert(ret0 is CheckStatus.Ok)

        /* check abbreviation collision error */
        val subsys1 = generateSubsystem("Abbrev", "Abbrev")
        val ret1 = judgments.checkIntroduceSubsystem(ctx, ctx, phi, rel, subsys1, true)
        assert(ret1 is CheckStatus.Error)
        ...
        val ret3 = judgments.checkIntroduceSubsystem(ctx, ctx, phi, rel, subsys3, true)
        ...
        val ret4 = judgments.checkIntroduceSubsystem(ctx1, ctx1, phi, rel, subsys4, true)
        assert(ret4 is CheckStatus.Ok)

        /* check invalid clientOf */
        // NOTE: why should this be invalid?
        ...
        val ret5 = judgments.checkIntroduceSubsystem(ctx, ctx, phi, rel, subsys5, true)
        assert(ret5 is CheckStatus.Ok)
    }
}
