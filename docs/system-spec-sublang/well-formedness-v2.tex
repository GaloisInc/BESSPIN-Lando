...
\begin{itemize}
\item \textbf{Judgement~\ref{j1}}
\begin{itemize}
\item $\wb{\env_0}{es}{\env_0}$: This premise checks the well-formedness of a list of elements es in the context of a top-level environment $\env_0$. It asserts that processing $es$ in $\env_0$ results in $\env_0$, which implies that the elements es are self-contained and do not modify the top-level environment.
...
\item $\wb{\env}{es}{\env''}$: Similarly, this checks the well-formedness of a list of elements $es$ in the same initial environment $\env$, resulting in another possibly modified environment $\env''$. It's assessing the collective impact of the remaining elements in the list after $e$.
...
\begin{itemize}
\item The same as Judgement~\ref{j4} except for systems without the abbreviation $n_a$
\end{itemize}
\end{itemize}
...
\begin{itemize}
\item This judgment defines the well-formedness criteria for a $\lab{Subsystem}$ element in the Lando language.
...
\begin{itemize}
\item The same as Judgement~\ref{j6} except for subsystems without the abbreviation $n_a$
\end{itemize}
\end{itemize}
...
\begin{itemize}
\item Same as Judgement~\ref{j8}, except for imported subsystems without an abbreviation $n_a$. As a result, we use the last part of the qualified name as the abbreviation of the subsystem.
\end{itemize}
\end{itemize}
...
\item Unclear what \textit{a missing option for name} means ($\lab{abbrev}=._{name}$)
\end{itemize}
\end{itemize}
...
\item \textbf{TODO: what do we do with the name?}
\end{itemize}
\end{itemize}
...
\begin{itemize}
\item \textbf{Judgement~\ref{j20}}
\begin{itemize}
\item An empty list of items is always well-formed
\end{itemize}
\end{itemize}
...
\begin{itemize}
\item \textbf{Judgement~\ref{j21}}
\begin{itemize}
\item See Judgement~\ref{j2} for detailed meaning of the premises.
\item $\wb{\env}{\conslist{i}{is}}{\env'' \uplus \env'''}$: The conclusion states that if both premises are true, then the entire list formed by concatenating $i$ and $is$is well-formed in the initial environment $\env$. The resulting environment after processing this concatenated list is the disjoint union of $\env''$ and $\env'''$ (denoted as $\env'' \uplus \env'''$).
\end{itemize}
\end{itemize}
...
\begin{itemize}
\item \textbf{Judgement~\ref{j22}} Item well-formed
\begin{itemize}
...
\begin{itemize}
\item \textbf{Judgement~\ref{j23}} Relation well-formed
\begin{itemize}
...
\begin{itemize}
\item \textbf{Judgement~\ref{j24}} Text well-formed
\begin{itemize}
...
\begin{itemize}
\item \textbf{Judgement~\ref{j27}}
\begin{itemize}
\item This rule states that a $\lab{SubSystem}$ can contain only $\lab{Subsystems}$, imported $\lab{Subsystems}$, $\lab{Components}$ or imported $\lab{Components}$
\end{itemize}
\end{itemize}
...
\begin{itemize}
\item \textbf{Judgement~\ref{j28}}
\begin{itemize}
\item This rule states that a $\lab{SubSystem}$ can contain only $\lab{Scenarios}$, $\lab{Requirements}$, $\lab{Events}$ or $\lab{Relations}$
\end{itemize}
\end{itemize}
